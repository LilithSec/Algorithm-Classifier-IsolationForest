...
subtest 'IF_OPT rejects an invalid value instead of passing it through' =>
    ...
    like( $out, qr/OPT_LEVEL=\Q$want\E\s*$/m,
        'falls back to the configured default opt level' );
...
subtest 'IF_ARCH adds -march=<value>' => sub {
    ...
    like( $out, qr/OPT_LEVEL=\Q$want\E\s*$/m,
        'OPT_LEVEL includes -march=x86-64-v2 (with -ffp-contract=off)' );
};
...
subtest 'IF_ARCH rejects an invalid value instead of passing it through' =>
    ...
    my $want = compose_flags( $DEF_OPT, $DEF_ARCH );
    like( $out, qr/OPT_LEVEL=\Q$want\E\s*$/m,
        'falls back to the configured default arch' );
...
subtest 'IF_ARCH takes precedence over IF_NATIVE when both are set' => sub {
    my ( $out, $status )
        = run_child( IF_NATIVE => 1, IF_ARCH => 'x86-64-v2' );
    is( $status, 0, 'child exits cleanly' ) or diag($out);
    my $want = compose_flags( $DEF_OPT, 'x86-64-v2' );
    like( $out, qr/OPT_LEVEL=\Q$want\E\s*$/m,
